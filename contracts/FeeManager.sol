...
contract FeeManager is IFeeManager {
    ...
    function claimFees(address claimer) public {
        ...
        uint256 rewardsToHarvest = FullMath.mulDiv(userBalance, accumulatedRewardsPerShare, REWARDS_PRECISION) - rewardDebt[claimer];
        // console.log("rewardsToHarvest", rewardsToHarvest);
        if (rewardsToHarvest == 0) {
            rewardDebt[claimer] = (FullMath.mulDiv(userBalance, accumulatedRewardsPerShare, REWARDS_PRECISION));
            return;
        }
    ...
    function _convertFeesToUSDC(address token0, uint256 fee0, address token1, uint256 fee1) internal returns (uint256 usdcFee) {
    ...
    function depositFees(address token0, uint256 fees0, address token1, uint256 fees1) public onlyVault {
        ...
        accumulatedRewardsPerShare = FullMath.mulDiv(rewards, REWARDS_PRECISION, vault.totalSupply());
    }
